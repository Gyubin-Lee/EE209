...
{
  char buf[MAX_STR_LEN + 2]; 
  int str1_len = StrGetLength(pcString1);
  int str2_len = StrGetLength(pcString2);
  int buf_len;
  char *point;

  /*argument validation*/
  if((str1_len > MAX_STR_LEN) || (str2_len > MAX_STR_LEN)){
    fprintf(stderr, "Error: argument is too long\n");
    return FALSE;
  }
  ...
  while (fgets(buf, sizeof(buf), stdin)) {
    ...
    while(*buf_point != '\0'){
        ...
        if(point != buf_point){
            ...
            printf("%s", pcString2);
            buf_point += StrGetLength(pcString1);
        }
    }
  }
...
DoDiff(const char *file1, const char *file2)
{
  ...
  /*open two files*/
  FILE *f1_point = fopen(file1, "r");
  FILE *f2_point = fopen(file2, "r");
  ...
  while(1){
    char *f1 = fgets(buf1, sizeof(buf1), f1_point);
    char *f2 = fgets(buf2, sizeof(buf2), f2_point);

    if(!f1 && f2){ //file1 ends first
      fprintf(stderr, "%s ends early at line %lu\n", file1, line);
      return FALSE;
    } else if(f1 && !f2){ //file2 ends first
      fprintf(stderr, "%s ends early at line %lu\n", file2, line);
      return FALSE;
	...
	*(f2+StrGetLength(f2)) = '\n';
      ...
      if((len = StrGetLength(f2)) > MAX_STR_LEN){
        fprintf(stderr, "Error: input line %s is too long\n", file2);
        return FALSE;
      }    
